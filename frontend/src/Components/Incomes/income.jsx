...
const IncomeForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const userId =sessionStorage.getItem('email'); // Assuming the user ID is stored in the 'id' field of the token
    ...
    const formData = {
      ...
      date: formattedDate,
      category,
      userId, // Include the user ID in the form data
    };

    try {
      const response = await fetch(`${process.env.REACT_APP_HIT_URL}/api/v1/add-income`, {
        method: 'POST',
        headers: {
      ...
      fetchTotalIncome(); // Update total income after adding new income
    } catch (error) {
